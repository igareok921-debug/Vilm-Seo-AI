...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      data: await getSearchConsoleRows(websiteId, "page"),
    });
  } catch (error) {
    const accessError = getWebsiteAccessErrorResponse(error);
    if (accessError) return accessError;
    console.error("[api/search-console/pages] Error:", error);
    return NextResponse.json({ error: "Search Console pages could not be loaded." }, { status: 500 });
  }
}
